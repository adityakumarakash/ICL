# Currently age protected for the adult dataset.
...
def maybe_download_adult(filename):
  ...
  if not tf.gfile.Exists(DATA_DIRECTORY + "raw"):
...
def process_adult_data():
  ...
  c_column_name = "age"   # For this adult dataset, age is the protected attribute.
  cutoff = train_data.shape[0]
  train_data = all_data.loc[:cutoff,\
    (all_data.columns != "income") & (all_data.columns != c_column_name)]
  train_c = all_data.loc[:cutoff,all_data.columns == c_column_name]
  train_labels = all_data.loc[:cutoff,all_data.columns == "income"]

  test_data = all_data.loc[cutoff:,\
    (all_data.columns != "income") & (all_data.columns != c_column_name)]
  test_c = all_data.loc[cutoff:,all_data.columns == c_column_name]
  ...
  mins = np.minimum(np.minimum(\
    train_data.min(axis=0),\
    val_data.min(axis=0)),\
    test_data.min(axis=0))

  train_data = (train_data - mins)/ (maxes - mins)
  test_data = (test_data - mins) / (maxes - mins)
  val_data = (val_data - mins) / (maxes - mins)
  # Note that the c-values are not being normalized and would be upon the reader 
  # of this data to take care of that.
...
if __name__ == "__main__":
  ...
  joblib.dump(
    value = ( train_data, split_numbers, train_size,\
      validation_data, validation_labels, validation_c,\
      test_data, test_labels, test_c ),\
    filename ="../data/adult_age_proc.z",\
    compress = ("zlib",1)\
  )
